...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const eventsRes = await fetch('http://localhost:8000/events', { method: 'GET' });
        const auditRes = await fetch('http://localhost:8000/audit', { method: 'GET' });
        const statusRes = await fetch('http://localhost:8000/status', { method: 'GET' });
